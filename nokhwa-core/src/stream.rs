...
use crate::{error::NokhwaError, frame_buffer::FrameBuffer, types::CameraFormat};
    ...
    pub fn next_event(&self) -> Result<Event, NokhwaError> {
        let event = match self.configuration.receiver {
            ...
            StreamReceiverBehaviour::Timeout(time) => self
                .frame
                .recv_timeout(time)
                .map_or_else(|_| Event::NotReady, |e| e),
            StreamReceiverBehaviour::Try => self.frame.try_recv().map_or_else(
                |why| match why {
    ...
    pub fn next_frame(&self) -> Result<FrameBuffer, NokhwaError> {
        loop {
            ...
            match event {
                ...
                Event::Error(error) => return Err(NokhwaError::ReadFrameError(error.to_string())),
            }
        }
    }
